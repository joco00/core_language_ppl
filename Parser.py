...
class Parser:
    ...
    def parse(self, token_list=None):
        if token_list: self.token_list = token_list
        funcName = 'parse'
        ...
        self.spaces += 1
        while self.frontToken(funcName) not in [Core.END, Core.ENDIF, Core.ENDWHILE, Core.ELSE]:
    ...
    def stmt(self):
        ...
        if type(self.token_list[0]) is str: n + self.getNonTerminal('<assign>')
        elif self.token_list[0] is Core.IF: n + self.getNonTerminal('<if>')
        elif self.token_list[0] is Core.WHILE: n + self.getNonTerminal('<loop>')
        elif self.token_list[0] is Core.INPUT: n + self.getNonTerminal('<in>')
        elif self.token_list[0] is Core.OUTPUT: n + self.getNonTerminal('<out>')
        elif self.token_list[0] is Core.INT: n + self.getNonTerminal('<decl>')
        elif self.token_list[0] is Core.BEGIN: n + self.getNonTerminal('<func>')
        else:
            self.terminal([str, Core.IF, Core.WHILE, Core.INPUT, Core.OUTPUT, Core.INT], funcName)
        return n
    ...
    def __if(self):
        ...
        self.scope.pop()
        n + self.terminal(Core.ENDIF, funcName) \
          + self.terminal(Core.SEMICOLON, funcName)
    ...
    def loop(self):
        ...
        while self.frontToken(funcName) not in [Core.END, Core.ENDIF, Core.ENDWHILE, Core.ELSE]:
            n + self.getNonTerminal('<stmt>')
        n + self.terminal(Core.ENDWHILE, funcName) \
          + self.terminal(Core.SEMICOLON, funcName)
        self.scope.pop()
        return n
    ...
    def cond(self):
        ...
        if self.token_list[0] is Core.NEGATION:
            n + self.terminal(Core.NEGATION, funcName) \
            ...
            if self.token_list[0] is Core.OR:
                n + self.terminal(Core.OR, funcName) \
                  + self.getNonTerminal('<cond>')
        return n
    ...
    def cmpr(self):
        ...
        if self.token_list[0] is Core.EQUAL: n + self.terminal(Core.EQUAL,funcName)
        elif self.token_list[0] is Core.LESS: n + self.terminal(Core.LESS,funcName)
        elif self.token_list[0] is Core.LESSEQUAL: n + self.terminal(Core.LESSEQUAL,funcName)
        else: self.terminal([Core.EQUAL, Core.LESS, Core.LESSEQUAL], funcName)
        n + self.getNonTerminal('<expr>')
        return n
    ...
    def expr(self):
        ...
        if self.token_list[0] is Core.ADD:
            n + self.terminal(Core.ADD, funcName) \
              + self.getNonTerminal('<expr>')
        elif self.token_list[0] is Core.SUB:
            n + self.terminal(Core.SUB, funcName) \
              + self.getNonTerminal('<expr>')
        return n
    ...
    def term(self):
        ...
        if self.token_list[0] is Core.MULT:
            n + self.terminal(Core.MULT, funcName) \
              + self.getNonTerminal('<term>')
        return n
    ...
    def factor(self):
        ...
        if type(self.token_list[0]) == str:
            ...
            n + self.terminal(Core.LPAREN, funcName) \
              +self.getNonTerminal('<expr>') \
              + self.terminal(Core.RPAREN, funcName)
        else:
    ...
    def decl_func(self):
        ...
        while self.frontToken(funcName) is not Core.ENDFUNC:
            n + self.getNonTerminal('<stmt>')
        n + self.terminal(Core.ENDFUNC, funcName) \
          + self.terminal(Core.SEMICOLON, funcName)
    ...
    def func(self):
        ...
        if name.type not in self.functions or len(signature) != len(self.functions[name.type]):
    ...
    def terminal(self, typ, func):
        ...
        if not (token is typ or type(token) is typ):
            ...
            sys.exit()
        if type(token) is Core: token = core_to_str(token)
        terminal = Node(token)
    ...
    def prettyPrintTree(self):
        for line in self.tree_str: print(line)

    """
        prints out the tree str
    """
    def prettPrintProgram(self):
        for line in self.program_str: (print(line))
